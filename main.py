...
import os
import sort
from abstract_class import OutputConsole, OutputFile
...
def main():
    ...
    output_console.output("Welcome to Virtual Assistant!")
    
    load_books(filename)    
    
    while True:
        output_console.output(f"\n{Colors.HEADER}Available commands: {Colors.UNDERLINE}{', '.join(command_maps.keys())}.{Colors.ENDC}")
        user_input = input("Enter the command: ").lower()
        if not user_input:
            output_console.output(f"{Colors.FAIL}{Colors.UNDERLINE}Error: Provide a command.{Colors.ENDC}")
            continue
        command = user_input.split()[0]

        if command in ["hello", "help", "showall", "congratulate", "search"]:
            output_console.output(command_maps[command]())
            continue
        elif command in ["bye", "good bye", "exit", "close"]:
            output_console.output(command_maps["bye"]())
            break
        elif user_input == "clean folder":
            folder = input("Enter the path of the folder to organize: ")
            output_console.output(command_maps["clean folder"](folder))
            continue
        elif command not in command_maps:
            output_console.output(f"{Colors.FAIL}{Colors.UNDERLINE}Error: Provide valid command.{Colors.ENDC}")
            continue
        else:
            output_console.output(f"{Colors.HEADER}Available options: {Colors.UNDERLINE}{', '.join(list(filter(lambda x: items_list[command][x] == True, items_list[command])))}{Colors.ENDC}")
            ...
            for item in items:
                if item == "note":
                    ...
                    output_console.output(command_maps[command]([item], name))
                    items.remove(item)
            if len(items) < 1:
                continue
            ...
            if item == "tags" and item in list(filter(lambda x: items_list[command][x] == True, items_list[command])):
                ...
                output_console.output(command_maps[command](items, name))
                ...
                output_console.output(f"{Colors.FAIL}{Colors.UNDERLINE}Error: choose from available options.{Colors.ENDC}")
